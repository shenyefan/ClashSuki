using ClashSuki.Services;
using ClashSuki.Utilities;
using ClashSuki.ViewModels;
using Microsoft.UI.Xaml.Controls;

namespace ClashSuki.Views;

public sealed partial class DnsPage : Page
{
    public DnsPage()
    {
        PageBinding.Bind(this, vm => vm.DnsVm);
        InitializeComponent();
        Loaded += DnsPage_Loaded;
    }

    private async void DnsPage_Loaded(object sender, Microsoft.UI.Xaml.RoutedEventArgs e)
    {
        if (DataContext is not DnsViewModel viewModel)
        {
            return;
        }

        await Task.Yield();
        try
        {
            await viewModel.LoadAsync();
        }
        catch (Exception ex)
        {
            viewModel.Runtime.Notifications.Error(
                "DNS 配置加载失败",
                source: LogSources.Dns,
                exception: ex);
        }
    }

    private async void Reset_Click(object sender, Microsoft.UI.Xaml.RoutedEventArgs e)
    {
        if (DataContext is not DnsViewModel viewModel)
        {
            return;
        }

        if (!await viewModel.Runtime.Notifications.ConfirmAsync(
                XamlRoot,
                "恢复默认 DNS 设置",
                "将恢复为默认值并保存到配置，随后触发热重载。",
                "恢复并保存",
                "取消",
                LogSources.Dns))
        {
            return;
        }

        await viewModel.ResetAndSaveCommand.ExecuteAsync(null);
    }
}
